"""
グラフ生成スクリプト。
data/<コード>.csv と config/stocks.csv を読み、docs/ に成果物を出力する。

出力：
- docs/compare.png   … 全銘柄の値動き比較（基準日=100 に揃えた指数化・note貼付用）
...
# 比較・銘柄別グラフに表示する期間（日数）。ここを変えると表示レンジを調整できる
NOTE_COMPARE_DAYS = 180
...
# ---------- 比較チャート（指数化・PNG） ----------
def build_compare_png(stocks, series_map, days):
...
def build_stock_png(s, df, days):
    ...
    if s.get("watch") is not None:
        plt.axhline(s["watch"], color="#f59e0b", linewidth=1.6, linestyle="--",
                    label=f"①様子見 {s['watch']:,}円")
    if s.get("exit") is not None:
        plt.axhline(s["exit"], color="#dc2626", linewidth=1.6, linestyle="--",
                    label=f"②撤退 {s['exit']:,}円")
    last = d.iloc[-1]
    plt.scatter([last["date"]], [last["close"]], color="#111827", zorder=5)
    plt.annotate(f"{last['close']:,.0f}円", (last["date"], last["close"]),
...
def fig_compare(stocks, series_map, days):
    fig = go.Figure()
    cutoff = pd.Timestamp.now() - pd.Timedelta(days=days)
    for i, s in enumerate(stocks):
        df = series_map.get(s["code"])
    ...
    fig.update_layout(title="値動き比較（基準日=100）", yaxis_title="指数",
...
def fig_stock(s, df, days):
    ...
    if s.get("exit") is not None:
        fig.add_hline(y=s["exit"], line_dash="dash", line_color="#dc2626",
                      ...
                      annotation_position="right")
    fig.update_layout(title=f"{s['name']}（{s['code']}）", yaxis_title="株価（円）",
                      template="plotly_white", height=420)
    return fig


def build_html(stocks, series_map, days):
    figs = [fig_compare(stocks, series_map, days)]
    for s in stocks:
        df = series_map.get(s["code"])
        if df is not None and not df.empty:
            figs.append(fig_stock(s, df, max(days, 250)))
...
<header>
  ...
  <p>各銘柄の値動きと①様子見／②撤退ライン ／ 最終更新：{today_str()}（JST）</p>
</header>
<main>
{charts_html}
  <p class="note">
...
def main():
    ...
    stocks = load_stocks(BASE)
    series_map = {s["code"]: load_series(s["code"]) for s in stocks}

    build_compare_png(stocks, series_map, NOTE_COMPARE_DAYS)
    for s in stocks:
        build_stock_png(s, series_map.get(s["code"]), max(NOTE_COMPARE_DAYS, 250))
    build_html(stocks, series_map, NOTE_COMPARE_DAYS)
