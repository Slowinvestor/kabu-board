"""
毎日の株価取得スクリプト。
- config/stocks.csv の各銘柄について、Yahoo!ファイナンスから日足（終値・出来高）を取得
...
# ここを変えると取得する履歴の長さを調整できる（例 "1y" "2y" "5y" "max"）
HISTORY_PERIOD = "2y"
...
def merge_save(code: str, fresh: pd.DataFrame) -> int:
    ...
    if os.path.exists(path):
        ...
        before = 0
        merged = fresh
    merged = merged.drop_duplicates(subset="date", keep="last")   # 同じ日付は新しい方を残す
...
def main():
    ...
    for s in stocks:
        try:
            fresh = fetch_one(s["ticker"], HISTORY_PERIOD)
    ...
    if errors and len(errors) == len(stocks):
